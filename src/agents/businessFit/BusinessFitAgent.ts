  ...
  private async persist(input: BusinessFitAgentInput, data: BusinessFitData): Promise<void> {
    try {
      await getSupabase().from('business_fit_checks').insert({
        ...
        competition_quality_score: data.competitionQualityScore,
        seller_competition_score: data.sellerCompetitionScore,
        exact_match_saturation_score: data.exactMatchSaturationScore,
        duplicate_listing_score: data.duplicateListingScore,
        price_compression_score: data.priceCompressionScore,
        same_source_likelihood_score: data.sameSourceLikelihoodScore,
        is_generic_commodity: data.isGenericCommodity,
        competition_gate_result: data.competitionGateResult,
        competition_rejection_reason: data.competitionRejectionReason ?? null,
        seller_count: input.sellerCount ?? null,
        exact_or_similar_match_count: input.exactOrSimilarMatchCount ?? null,
        duplicate_ratio: input.duplicateRatio ?? null,
